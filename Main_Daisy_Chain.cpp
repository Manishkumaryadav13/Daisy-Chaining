...
Dynamixel2Pico::Dynamixel2Pico(uint16_t packet_buf_size)
: Master(2.0, packet_buf_size), model_number_idx_last_index_(0)
{
  memset(&model_number_idx_, 0xff, sizeof(model_number_idx_));
}

Dynamixel2Pico::Dynamixel2Pico(uart_inst_t *uart, int dir_pin, int rx_pin, int tx_pin, uint16_t packet_buf_size)
: Master(2.0, packet_buf_size), model_number_idx_last_index_(0)
{
  p_dxl_port_ = new SerialPortHandler(uart, dir_pin, rx_pin, tx_pin);
  setPort(p_dxl_port_);
  memset(&model_number_idx_, 0xff, sizeof(model_number_idx_));
}

/* For Master configuration */
void Dynamixel2Pico::begin(unsigned long baud)
{
  p_dxl_port_ = (SerialPortHandler*)getPort();
  if(p_dxl_port_ == nullptr){
    setLastLibErrCode(D2A_LIB_ERROR_NULLPTR_PORT_HANDLER);
    printf("Invalid port handler\n");
    sleep_ms(1000);
    return;
  }
...
unsigned long Dynamixel2Pico::getPortBaud()
{
  p_dxl_port_ = (SerialPortHandler*)getPort();
...
bool Dynamixel2Pico::ping(uint8_t id)
{
  bool ret = false;
...
bool 
Dynamixel2Pico::setModelNumber(uint8_t id, uint16_t model_number)
{
  bool ret = false;
...
uint16_t Dynamixel2Pico::getModelNumber(uint8_t id)
{
...
bool Dynamixel2Pico::setID(uint8_t id, uint8_t new_id)
{
  return writeControlTableItem(ControlTableItem::ID, id, new_id);
}

bool Dynamixel2Pico::setProtocol(uint8_t id, float version)
{
  uint8_t ver_idx;
...
bool Dynamixel2Pico::setBaudrate(uint8_t id, uint32_t baudrate)
{
...
/* Commands for Slave */
bool Dynamixel2Pico::torqueOn(uint8_t id)
{
  return setTorqueEnable(id, true);
}

bool Dynamixel2Pico::torqueOff(uint8_t id)
{
  return setTorqueEnable(id, false);
}

bool Dynamixel2Pico::setTorqueEnable(uint8_t id, bool enable)
{
  return writeControlTableItem(ControlTableItem::TORQUE_ENABLE, id, enable);
}

bool Dynamixel2Pico::ledOn(uint8_t id)
{
  return setLedState(id, true);
}

bool Dynamixel2Pico::ledOff(uint8_t id)
{
  return setLedState(id, false);
}

bool Dynamixel2Pico::setLedState(uint8_t id, bool state)
{
  bool ret = false;
  uint16_t model_num = getModelNumberFromTable(id);
...
//TODO: Simplify the code by grouping model numbers.
bool Dynamixel2Pico::setOperatingMode(uint8_t id, uint8_t mode)
{
  bool ret = false;
  uint16_t model_num = getModelNumberFromTable(id);
...
bool Dynamixel2Pico::setGoalPosition(uint8_t id, float value, uint8_t unit)
{
...
float Dynamixel2Pico::getPresentPosition(uint8_t id, uint8_t unit)
{
  if(unit != UNIT_RAW && unit != UNIT_DEGREE)
    return 0.0;
...
bool Dynamixel2Pico::setGoalVelocity(uint8_t id, float value, uint8_t unit)
{
...
float Dynamixel2Pico::getPresentVelocity(uint8_t id, uint8_t unit)
{
  if(unit != UNIT_RAW && unit != UNIT_PERCENT && unit != UNIT_RPM)
    return 0.0;
...
bool Dynamixel2Pico::setGoalPWM(uint8_t id, float value, uint8_t unit)
{
...
float Dynamixel2Pico::getPresentPWM(uint8_t id, uint8_t unit)
{
  if(unit != UNIT_RAW && unit != UNIT_PERCENT)
    return 0.0;
...
bool Dynamixel2Pico::setGoalCurrent(uint8_t id, float value, uint8_t unit)
{
...
float Dynamixel2Pico::getPresentCurrent(uint8_t id, uint8_t unit)
{
  if(unit != UNIT_RAW && unit != UNIT_PERCENT && unit != UNIT_MILLI_AMPERE)
    return 0.0;
...
bool Dynamixel2Pico::getTorqueEnableStat(uint8_t id)
{
  bool ret = false;
...
int32_t Dynamixel2Pico::readControlTableItem(uint8_t item_idx, uint8_t id, uint32_t timeout)
{
  int32_t ret = 0;
  uint16_t model_num = getModelNumberFromTable(id);
...
bool Dynamixel2Pico::writeControlTableItem(uint8_t item_idx, uint8_t id, int32_t data, uint32_t timeout)
{
  bool ret = false;
  uint16_t model_num = getModelNumberFromTable(id);
...
int32_t Dynamixel2Pico::readControlTableItem(uint16_t model_num, uint8_t item_idx, uint8_t id, uint32_t timeout)
{
...
bool Dynamixel2Pico::writeControlTableItem(uint16_t model_num, uint8_t item_idx, uint8_t id, int32_t data, uint32_t timeout)
{
  bool ret = false;
  ControlTableItemInfo_t item_info;
...
uint8_t
Dynamixel2Pico::getModelNumberIndex(uint16_t model_num)
{
  uint8_t i, ret = 0xFF;
...
uint16_t Dynamixel2Pico::getModelNumberFromTable(uint8_t id)
{
  uint8_t idx;
  uint16_t model_num;
...
float Dynamixel2Pico::readForRangeDependencyFunc(uint8_t func_idx, uint8_t id, uint8_t unit)
{
  float ret = 0;
  int32_t recv_data = 0;
  uint16_t model_num = getModelNumberFromTable(id);
...
bool Dynamixel2Pico::writeForRangeDependencyFunc(uint8_t func_idx, uint8_t id, float value, uint8_t unit)
{
  bool ret = false;
  int32_t data = 0;
  uint16_t model_num = getModelNumberFromTable(id);
